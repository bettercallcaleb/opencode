...
describe("enterprise MCP admission", () => {
  ...
  test("makes connection eligibility explicit only in connect mode", () => {
    const result = diagnose({ policy: { ...enterpriseMcpPolicy, mode: "connect" } })
    expect(result.summary).toMatchObject({ status: "pass", operationallyEnabled: true })
    expect(result.checks.map((check) => check.code)).toContain("MCP_POLICY_MODE_CONNECT")
    expect(result.references[0].checks.map((check) => check.code)).toContain("MCP_CONNECTION_ELIGIBLE")
  })
  ...
  test("fails unknown and unmanaged definitions", () => {
    ...
    expect(
      diagnose({
        references: { source: { type: "remote", url: "https://mcp.corp.example" } },
      }).references[0]?.checks.map((check) => check.code),
    ).toContain("MCP_DEFINITION_UNMANAGED")
  })
  ...
  test("enforces project reference policy and detects namespaces", () => {
    ...
    expect(collision.references.flatMap((item) => item.checks).map((check) => check.code)).toContain(
      "MCP_NAME_COLLISION",
    )
  })
  ...
  test("rejects URL credentials, query, fragment, scheme and unresolved placeholders", () => {
    ...
    for (const [url, code] of urls) {
      const policy = {
        ...enterpriseMcpPolicy,
        servers: { "source-control": { ...enterpriseMcpPolicy.servers["source-control"], url } },
      }
  ...
  test("rejects header case collisions, missing values, invalid names, and forbidden transport headers", () => {
    const variants = [
      {
        allowedNames: ["Authorization", "authorization"],
        values: { authorization: { source: "environment" as const, name: "TOKEN", format: "Bearer" as const } },
      },
      { allowedNames: ["Authorization"], values: {} },
      {
        allowedNames: [" Authorization"],
        values: { " Authorization": { source: "environment" as const, name: "TOKEN", format: "Raw" as const } },
      },
      {
        allowedNames: [":authority"],
        values: { ":authority": { source: "environment" as const, name: "TOKEN", format: "Raw" as const } },
      },
      ...[
        "Host",
        "Content-Length",
    ...
    for (const headers of variants) {
      const policy = {
        ...enterpriseMcpPolicy,
        servers: { "source-control": { ...enterpriseMcpPolicy.servers["source-control"], headers } },
      }
      expect(diagnose({ policy }).summary.status).toBe("fail")
    }
  })
  ...
  test.each([
    ...
    [
      {
        kind: "managed-preference" as const,
        source: "mobileconfig:/Library/Managed Preferences/ai.opencode.managed.plist",
      },
      "darwin" as const,
    ],
    ...
    expect(result.checks.map((check) => check.code)).toContain(
      policySource.kind === "managed-preference"
  ...
  test("ignores untrusted policy fields when a later managed policy exists", () => {
    ...
    expect(result.checks).toContainEqual(
      expect.objectContaining({ code: "MCP_POLICY_SOURCE_UNTRUSTED", status: "warning" }),
    )
  })
})
